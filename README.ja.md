...
統合作業中です。loadout-osは、カーネルと以前は個別のパッケージとして存在していた2つのアダプター、およびライブランタイムフックを1つにまとめます。本日公開されているアップストリームは**`@mcptoolshop/ai-loadout`**（カーネル）です。統合された`loadout-os`パッケージはこのリポジトリからリリースされます。3つのレガシーバイナリは、計画された廃止まで引き続き機能します。
